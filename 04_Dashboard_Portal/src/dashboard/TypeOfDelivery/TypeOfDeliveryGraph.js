...
import TypeOfDeliveryLineGraph from "./TypeOfDeliveryLineGraph";
import TypeOfDeliveryPieGraph from "./TypeOfDeliveryPieGraph";
...
let cesarean_delivery_count = [], xaxis_dates = [], normal_delivery_count = [], rangedates = [], sum_normal_delivery_count = 0, sum_cesarean_delivery_count = 0;
let startDate = '', endDate = '', quarterCal = '', errors = false;
export default class TypeOfDeliveryGraph extends Component {
    constructor(props) {
        super(props);
    ...
    apply = function () {
        ...
        cesarean_delivery_count = []; xaxis_dates = []; normal_delivery_count = [];
        rangedates = []; sum_normal_delivery_count = 0; sum_cesarean_delivery_count = 0;
        if (errors == false) {
            axios.get(
                'http://localhost:8080/api/typeOfDeliveryGraph?range=' + this.state.range + '&sepsis_status=' + this.state.sepsisStatus + '&date_from=' + this.state.startDate + '&date_to=' + this.state.endDate,
                    ...
                    for (var d = 0; d < rangedates.length; d++) {
                        for (var i = 0; i < this.state.data.length; i++) {
                            if (rangedates[d] === this.state.data[i].baby_adm_date) {
                                if (this.state.data[i].type_of_delivery == "Cesarean") {
                                    cesarean_delivery_count[d] = this.state.data[i].count;
                                }
                                if (this.state.data[i].type_of_delivery == "Normal") {
                                    normal_delivery_count[d] = this.state.data[i].count;
                                }
                            }
                            else {
                                if (cesarean_delivery_count[d] == null) {
                                    cesarean_delivery_count[d] = 0;
                                }
                                if (normal_delivery_count[d] == null) {
                                    normal_delivery_count[d] = 0;
                                }
                            }
                        }
                    }
                    console.log(cesarean_delivery_count);
                    console.log(normal_delivery_count);
                    for (var i = 0; i < cesarean_delivery_count.length; i++) {
                        sum_normal_delivery_count = sum_normal_delivery_count + cesarean_delivery_count[i];
                    }
                    for (var i = 0; i < normal_delivery_count.length; i++) {
                        sum_cesarean_delivery_count = sum_cesarean_delivery_count + normal_delivery_count[i];
                    }
                    console.log(sum_normal_delivery_count, sum_cesarean_delivery_count);
                ...
                {this.state.isLoaded ?
                    <div >
                        {this.state.data.length == 0 ?
                            ...
                            <Row gutter={16}>
                                <Col span={12}>
                                    <Card title="Percentage of Type of Delivery"
                                        style={{ marginTop: '2%' }}>
                                        <TypeOfDeliveryPieGraph sum_normal_delivery_count={sum_normal_delivery_count}
                                            sum_cesarean_delivery_count={sum_cesarean_delivery_count} />
                                    </Card>
                                </Col>
                                <Col span={12}>
                                    <Card title="Type of Delivery" style={{ marginTop: '2%' }}>
                                        <TypeOfDeliveryLineGraph cesarean_delivery_count={cesarean_delivery_count}
                                            normal_delivery_count={normal_delivery_count}
                                            xaxis_dates={rangedates}
                                        >
                                        </TypeOfDeliveryLineGraph>
                                    </Card>
                                </Col>
                            </Row>}
                    </div>
