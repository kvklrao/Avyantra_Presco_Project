...
export default class EosLosGraph extends Component {
    ...
    apply = function () {
        ...
        if (errors == false) {
            axios.get(
                'http://localhost:8080/api/eosLosGraph?range=' + this.state.range+'&date_from=' + this.state.startDate + '&date_to=' + this.state.endDate,
                ...
                if (this.state.range == 1) {
                    ...
                    for (var d = 0; d < rangedates.length; d++) {
                        for (var i = 0; i < this.state.data.length; i++) {
                            if (rangedates[d] === this.state.data[i].baby_adm_date) {
                                if (this.state.data[i].baby_condition_yes_eos_los == "Los") {
                                    los_count[d] = this.state.data[i].count;
                                }
                                if (this.state.data[i].baby_condition_yes_eos_los == "Eos") {
                                    eos_count[d] = this.state.data[i].count;
                                }
                            }
                            else {
                                if (los_count[d] == null) {
                    ...
                    console.log(los_count);
                    console.log(eos_count);
                    for (var i = 0; i < los_count.length; i++) {
                        sum_eos_count = sum_eos_count + los_count[i];
                    }
                    for (var i = 0; i < eos_count.length; i++) {
                        sum_los_count = sum_los_count + eos_count[i];
                    }
                    console.log(sum_eos_count, sum_los_count);
                ...
                {this.state.isLoaded ?
                    <div >
                        {this.state.data.length == 0 ?
                            ...
                            <Row gutter={16}>
                                <Col span={12}>
                                    <Card title="Percentage of EOS LOS"
                                        style={{ marginTop: '2%' }}>
                                ...
                                <Col span={12}>
                                    <Card title="EOS LOS" style={{ marginTop: '2%' }}>
