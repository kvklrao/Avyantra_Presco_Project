...
const chart1 = async (req, res) => {
  ...
  where patient_baby_investigations.blood_culture_report <> 'NA' and
   str_to_date(patient_general_infos.baby_date_of_admission, '%d/%m/%Y') between '${fromDate}' and '${toDate}'
   and dashboard_users.user_id = '${userId}'
  GROUP BY patient_general_infos.baby_date_of_admission ,patient_baby_investigations.blood_culture_report
  order by baby_adm_date,patient_baby_investigations.blood_culture_report`, {
   ...
   res.json({ results });
     ...
     patient_baby_investigations.blood_culture_report
     order by baby_adm_date, patient_baby_investigations.blood_culture_report`, {
     });
