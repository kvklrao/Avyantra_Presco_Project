...
const typeOfDeliveryGraph = async (req, res) => {
  ...
  if (range == 0 || range == 1) {
    ...
    sum(case when IsNormal = 1 then 1 else 0 end) as normal_delivery_count,
    sum(case when IsNormal = 0 then 1 else 0 end) as cesarean_delivery_count,
    hospital_id,hospital_branch_id from (SELECT
    pbi.id,
    str_to_date(pgi.baby_date_of_admission, '%d/%m/%Y') as baby_adm_date,
    pmi.type_of_delivery type_of_delivery,
    ...
    case when
    sum(case when pgi.baby_preterm = 'Normal' then 1 else 0 end) >0 then 1 else 0 end as IsNormal
    FROM
