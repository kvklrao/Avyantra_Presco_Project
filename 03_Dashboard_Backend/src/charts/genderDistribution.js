...
const GenderDistribution = async (req, res) => {
  ...
  if (range == 0 || range == 1) {
    ...
    sum(case when IsGenderMale = 1 then 1 else 0 end) as male_count,
    sum(case when IsGenderMale = 0 then 1 else 0 end) as female_count,
    hospital_id,hospital_branch_id from (SELECT
    pbi.id,
    str_to_date(pgi.baby_date_of_admission, '%d/%m/%Y') as baby_adm_date,
    pgi.baby_gender as gender,
    ...
    case when
    sum(case when pgi.baby_gender = 'Male' then 1 else 0 end) >0 then 1 else 0 end as IsGenderMale
    FROM
    ...
    ${hospital_query}
    ${branch_query}
    and pbin.blood_culture_report = '${sepsisStatus}'
    group by
    pbi.id,
    ...
    const [results2, metadata] = await sequelize.query(`select baby_adm_date,
    ${chartType},sum(case when IsSepsisPositive = 1 then 1 else 0 end) as sepsis_positive_count,
    sum(case when IsSepsisPositive = 0 then 1 else 0 end) sepsis_negative_count,
    sum(case when IsGenderMale = 1 then 1 else 0 end) as male_count,
    sum(case when IsGenderMale = 0 then 1 else 0 end) as female_count,
    hospital_id,hospital_branch_id from (SELECT
