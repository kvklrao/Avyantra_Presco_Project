...
const PreTermGraph = async (req, res) => {
  ...
  if (range == 0 || range == 1) {
    ...
    sum(case when IsPreTermYes = 1 then 1 else 0 end) as preterm_yes,
    sum(case when IsPreTermYes = 0 then 1 else 0 end) as preterm_no,
    ...
    case when
    sum(case when pgi.baby_preterm = 'Yes' then 1 else 0 end) >0 then 1 else 0 end as IsPreTermYes
    FROM
