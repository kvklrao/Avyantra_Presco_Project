...
exports.hospitalSignUp = async (req,res,next)=>{
    ...
    let msg = {
        ...
        subject: 'New Hospital user registration.',
        text: `User email is ${req.body.email}`,
        html: `User email is <strong>${req.body.email}</strong>`,
    };
            ...
            pReadingModels.user_model.findOne({
                ...
                .then(async result => {
                    ...
                    try{
                        ...
                        authUsers = await auth0.getUsersByEmail(result.dataValues.email_address);
                    
                        for (let i = 0; i< authUsers.length; i++){
                            authRes = await auth0.deleteUser({id: authUsers[i].user_id });
                        }
                        
                        authRes = await auth0.createUser(auth0_params);
