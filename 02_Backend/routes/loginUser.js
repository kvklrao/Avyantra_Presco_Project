...
module.exports = app => {
    ...
    if (req.body.email.match(mailformat)) {
      pReadingModels.user_model.findOne({
        ...
        if (!result) {
          ...
          const url = constant.ip_address + '/#/reset_password/' + passcode;
          ...
          let msg = {
            ...
            html: `<b> Hi </b>` +
              ...
              `<br>` + url + `</br>` +
              `<br>If you don't want to change your password, you ignore this email.</br>
  ...
  app.post('/resetPassword', (req, res) => {
    ...
    .then( async result=> {
        ...
        authUsers = await auth0.getUsersByEmail(result.email_address);

        for (let i = 0; i< authUsers.length; i++){
            authRes = await auth0.updateUser({id: authUsers[i].user_id }, auth0_params);

            if(!authRes) {
              res.json(responseHelper.serveError("User not found, Please contact custumer support",{}));
              return;
            }

            if(authRes.blocked) {
              res.json(responseHelper.serveError("User is blocked, Please contact custumer support",{}));
              return;
            }
        }
